...
public class ListarFactura extends JFrame {
	 ...
	 * 
	 * if(v==null) { switch(cbxFiltro.getSelectedIndex()) { case 0:
	 * model.setRowCount(0); fil = new Object [model.getColumnCount()];
	 * 
	 * for(Venta fact : Tienda.getInstance().getFacturas()){
	 * 
	 * fil[0] = fact.getIdFactura(); fil[1] = fact.getClienteVenta().getNombre();
	 * fil[2] = fact.getVendedorVenta().getNombre(); fil[3] = fact.getMontoTotal();
	 * fil[4] = fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
	 * model.addRow(fil); } break; case 1: model.setRowCount(0); fil = new Object
	 * [model.getColumnCount()];
	 * 
	 * for(Venta fact : Tienda.getInstance().getFacturas()){ fil[0] =
	 * fact.getIdFactura(); fil[1] = fact.getClienteVenta().getNombre(); fil[2] =
	 * fact.getVendedorVenta().getNombre(); fil[3] = fact.getMontoTotal(); fil[4] =
	 * fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
	 * model.addRow(fil); } } break; case 2: model.setRowCount(0); fil = new Object
	 * [model.getColumnCount()]; for(Venta fact :
	 * Tienda.getInstance().getFacturas()){ fil[0] = fact.getIdFactura(); fil[1] =
	 * fact.getClienteVenta().getNombre(); fil[2] =
	 * fact.getVendedorVenta().getNombre(); fil[3] = fact.getMontoTotal(); fil[4] =
	 * fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
	 * model.addRow(fil); } } break; case 3:
	 * 
	 * model.setRowCount(0); fil = new Object [model.getColumnCount()];
	 * 
	 * for(Venta fact : Tienda.getInstance().getFacturas()){ fil[0] =
	 ...
	 * fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
	 * model.addRow(fil); } } break;
	 * 
	 * }
	 * 
	 * }else {
	 * 
	 * switch(cbxFiltro.getSelectedIndex()) { case 0: model.setRowCount(0); fil =
	 * new Object [model.getColumnCount()];
	 * 
	 * for(Venta fact : Tienda.getInstance().getFacturas()){
	 * if(fact.getVendedorVenta().equals(v)) { fil[0] = fact.getIdFactura(); fil[1]
	 * = fact.getClienteVenta().getNombre(); fil[2] =
	 * fact.getVendedorVenta().getNombre(); fil[3] = fact.getMontoTotal(); fil[4] =
	 * fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
	 ...
	 * model.setRowCount(0); fil = new Object [model.getColumnCount()];
	 * 
	 * for(Venta fact : Tienda.getInstance().getFacturas() ){ if(fact.isEstado() &&
	 * fact.vencida() && fact.getVendedorVenta().equals(v)) { fil[0] =
	 * fact.getIdFactura(); fil[1] = fact.getClienteVenta().getNombre(); fil[2] =
	 * fact.getVendedorVenta().getNombre(); fil[3] = fact.getMontoTotal(); fil[4] =
	 * fact.getFecha(); fil[5] =
	 * fact.getComponentesVenta().size()+fact.getCombosVenta().size();
